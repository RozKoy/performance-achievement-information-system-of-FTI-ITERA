        ...
        if (count($years)) {
            ...
            $data = $yearInstance->sasaranStrategis()
                ->whereHas('indikatorKinerja', function (Builder $query) use ($statusIndex, $periodInstance, $user): void {
                    ...
                    $query->when(
                        $statusIndex === 1,
                        function (Builder $query) use ($periodInstance, $user): Builder {
                            return $query->whereDoesntHave('realization', function (Builder $query) use ($periodInstance, $user): void {
                    ...
                    $query->when(
                        $statusIndex === 2,
                        function (Builder $query) use ($periodInstance, $user): Builder {
                            return $query->whereHas('realization', function (Builder $query) use ($periodInstance, $user): void {
                                $query->whereBelongsTo($user->unit)
                                    ->whereBelongsTo($periodInstance, 'period');
                            });
                        }
