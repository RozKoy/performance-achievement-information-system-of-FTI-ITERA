<nav class="flex" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 rtl:space-x-reverse md:space-x-2">
        <li title="Halaman Beranda" class="inline-flex items-center">
            <a href="{{ url(route('super-admin-dashboard')) }}" class="@if (request()->routeIs('super-admin-dashboard')) pointer-events-none text-primary/75 @else text-gray-500 hover:text-primary @endif inline-flex items-center text-sm font-medium">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="me-2.5 h-3 w-3">
            ...
            @foreach ($breadCrumbs as $item)
                <li title="Halaman {{ $item['name'] }}">
                    <div class="flex items-center">
                        ...
                        <a href="{{ url(route($item['link'])) }}" class="@if (request()->routeIs($item['link'])) pointer-events-none text-primary/75 @else text-gray-500 hover:text-primary @endif ms-1 text-sm font-medium md:ms-2">{{ $item['name'] }}</a>
                    </div>
                </li>
                @if (request()->routeIs($item['link']))
                @break
            @endif
        @endforeach
    @endisset
