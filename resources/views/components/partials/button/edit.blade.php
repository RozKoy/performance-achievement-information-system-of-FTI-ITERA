@if (isset($link))
    <a href="{{ url($link) }}" title="Ubah" class="{{ isset($style) ? $style : '' }} rounded-full bg-yellow-500 p-0.5 text-white hover:bg-yellow-400">
...
@else
    <button type="submit" title="Tombol ubah" class="ml-auto flex items-center gap-1 rounded-lg bg-yellow-500 px-2 py-1.5 text-center text-xs text-white hover:bg-yellow-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 max-sm:w-fit sm:text-sm">
