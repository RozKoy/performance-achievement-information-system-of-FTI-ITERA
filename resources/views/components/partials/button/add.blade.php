<button type="{{ isset($submit) ? 'submit' : 'button' }}" @if (!isset($submit) && (isset($href) || isset($route))) onclick="moveTo('{{ url(isset($href) ? route($href) : $route) }}')" @endif title="Tombol tambah" class="{{ isset($style) ? $style : '' }} flex items-center gap-1 rounded-lg bg-green-500 px-2 py-1.5 text-center text-xs text-white hover:bg-green-400 focus:outline-none focus:ring-2 focus:ring-green-400 max-sm:w-fit sm:text-sm">
...
@if (!isset($submit) && (isset($href) || isset($route)))
    @pushOnce('script')
        <script>
