<button type="{{ isset($submit) ? 'submit' : 'button' }}" @if (!isset($submit) && isset($href)) onclick="moveTo('{{ url(route($href)) }}')" @endif title="Tombol tambah" class="ml-auto flex items-center gap-1 rounded-lg bg-green-500 px-2 py-1.5 text-center text-xs text-white hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-green-600 max-sm:w-fit sm:text-sm">
