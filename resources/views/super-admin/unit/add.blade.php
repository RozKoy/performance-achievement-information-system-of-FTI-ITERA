...
<x-super-admin-template title="Tambah Unit - Super Admin">
    ...
    <form action="" method="POST" class="flex flex-col gap-2">
        ...
        <x-partials.input.text name="short_name" title="Nama pendek unit (contoh : if)" value="{{ old('short_name') }}" autofocus required />
        <p class="text-sm sm:text-base">Pilih Pengguna</p>
        <div class="flex flex-wrap gap-1 *:rounded-lg *:border">

            @foreach ($users as $user)
                <div class="relative flex min-w-40 flex-1 items-center gap-1.5 px-2 py-1">
                    ...
                    <label for="user-{{ $loop->iteration }}" class="flex-1 overflow-hidden text-xs text-primary *:truncate sm:text-sm">
                        <div class="flex items-center gap-1 font-semibold">
                            <p title="{{ $user['access'] === \App\Models\User::ACCESS_EDITOR ? 'Semua akses' : 'Hanya melihat' }}">

                                @if ($user['access'] === \App\Models\User::ACCESS_EDITOR)
