<x-super-admin-template title="Beranda - Super Admin">
    <div class="flex w-full items-center justify-center gap-3 text-primary max-lg:flex-wrap">
        <div
            class="flex w-1/2 max-w-screen-md flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-primary/75 p-3 shadow shadow-primary max-lg:w-full">
            <div class="flex w-full items-center justify-between">
                <h6 class="text-lg uppercase md:text-xl" title="Rencana Strategis">
                    Rencana Strategis
                </h6>
                <form action="">
            ...
            <div class="relative flex aspect-square w-3/4 max-w-screen-sm items-center justify-center">
                ...
                <p
                    class="{{ $rsPercent >= 75 ? 'text-green-500' : ($rsPercent >= 50 ? 'text-yellow-500' : 'text-red-500') }} absolute pt-7 text-3xl max-md:text-xl">
                    {{ $rsPercent }}%
                </p>
            </div>
            <div class="flex w-full items-center justify-between">
                <a href="{{ route('super-admin-dashboard-rs-export', ['year' => $rsYear]) }}" title="Unduh Excel"
                    target="_blank"
                    class="mr-auto flex items-center gap-1 rounded-lg border px-1.5 py-1 text-sm text-green-500 hover:bg-slate-50 max-md:text-xs">
                ...
                <a href="{{ url(route('super-admin-dashboard-rs', ['year' => $rsYear])) }}"
                    class="ml-auto inline-flex items-center font-medium text-primary underline hover:text-primary/80 max-md:text-sm">
                    Selengkapnya
                    <svg class="ms-1 aspect-square w-2.5 max-md:w-2" aria-hidden="true"
        ...
        <div
            class="flex w-1/2 max-w-screen-md flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-primary/75 p-3 shadow shadow-primary max-lg:w-full">
            <div class="flex w-full items-center justify-between">
                <h6 class="text-lg uppercase md:text-xl" title="Indikator Kinerja Utama">
            ...
            <div class="relative flex aspect-square w-3/4 max-w-screen-sm items-center justify-center">
                ...
                <p
                    class="{{ $ikuPercent >= 75 ? 'text-green-500' : ($ikuPercent >= 50 ? 'text-yellow-500' : 'text-red-500') }} absolute pt-7 text-3xl max-md:text-xl">
                    {{ $ikuPercent }}%
                </p>
            </div>
            <div class="flex w-full items-center justify-between">
                <a href="{{ route('super-admin-dashboard-iku-export', ['year' => $ikuYear]) }}" title="Unduh Excel"
                    target="_blank"
                    class="flex items-center gap-1 rounded-lg border px-1.5 py-1 text-sm text-green-500 hover:bg-slate-50 max-md:text-xs">
                    <img src="{{ url(asset('storage/assets/icons/excel.png')) }}" alt="Excel"
                        class="w-7 max-md:w-6">
                    Unduh
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                        class="aspect-square w-2.5 max-md:w-2">
                        <g>
                ...
                <a href="{{ url(route('super-admin-dashboard-iku', ['year' => $ikuYear])) }}"
                    class="ml-auto inline-flex items-center font-medium text-primary underline hover:text-primary/80 max-md:text-sm">
                    Selengkapnya
                    <svg class="ms-1 aspect-square w-2.5 max-md:w-2" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
    ...
    @if (count($rsIndikatorKinerja))
        <div
            class="flex w-full cursor-default flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-primary/75 p-3 text-primary shadow shadow-primary">
            <h6 class="uppercase">
                Rencana Strategis
            </h6>
                    ...
                    <tbody>
                        ...
                        @foreach ($rsIndikatorKinerja as $item)
                            <tr class="*:border *:border-primary *:p-1 *:text-center">
                                <th title="{{ $item['name'] }}" class="max-w-96 overflow-hidden truncate text-left">
                                    {{ $item['name'] }}
                                </th>
                                    ...
                                    @if ($realization->count())
                                        @if ($item['type'] === 'teks')
                                            @php
                                                $tempArray = [];
                                                foreach (
                                                    $realization->pluck('realization')->toArray()
                                                    as $tempRealization
                                                ) {
                                        ...
                                        @else
                                            @php
                                                ...
                                                $realization =
                                                    $item['type'] === 'angka'
                                                        ? $realization->sum('realization')
                                                        : $realization->average('realization');

                                                if (
                                                    !$target ||
                                                    $realization >= (float) ($target['target'] ?? $realization + 1)
                                                ) {
                                                    $percent = 100;
                                                    $status = true;
        ...
        <div
            ...
            <div class="w-full overflow-x-auto">
                <table class="min-w-full">
                    <thead>
                        <tr class="*:border *:border-primary *:p-1">
                            <th title="Indikator Kinerja Program">
                                ...
                                @foreach ($units as $unit)
                                    @php
                                        $unitStatus = collect($item['unitStatus'])
                                            ->where('status', \App\Models\IKUUnitStatus::STATUS_BLANK)
                                            ->where('unit_id', $unit['id'])
                                            ->count();
                                        $singleAchievements = collect($item['singleAchievements'])->where(
                                            'unit_id',
                                            $unit['id'],
                                        );
                                    ...
                                    @endphp
                                    <td
                                        class="{{ $status === 2 ? 'bg-green-300' : ($status === 1 ? 'bg-red-300' : '') }}">
                                        @if ($percent !== null)
                                            {{ $percent }}%
                                        @endif
                                    </td>
    ...
    @push('script')
        ...
        <script>
            const setChart = (canvas, data, percent) => {
                ...
                let backgroundColor = data?.sum ? [percent >= 75 ? color.green : (percent >= 50 ? color.yellow : color.red),
                    color.grey
                ] : [color.red];
